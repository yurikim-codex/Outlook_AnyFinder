...
> **최종 갱신일**: 2026-06-07  
> **대상 버전**: Ver0.9.1 → 0.9.1 (2026-06-07 최적화 보완)  
...
## 9. v0.9.1 최적화 보완 (2026-06-07)

### 9.1 해결된 주요 이슈

| 이슈 | 원인 | 해결 |
|---|---|---|
| **재동기화 시 최신 메일 누락** | `_scan_items` `>` vs `_iter_folder` `>=` 불일치, 폴더명 별칭 누락, 하위폴더 false-positive | Restrict 통일, 별칭 중앙화, 전체 DB 비교 |
| **총합 ≠ 폴더별 합계** | 하위 폴더 이메일이 사이드바 4대 폴더 집계에서 누락 | `└ 폴더편지함` 행 추가, `total_count` 교차검증 |
| **폴더 이동 미감지** | `compute_scan_hash`에 `folder_name` 미포함 | `folder_name` 추가 → 해시 불일치로 `updated` 감지 |
| **삭제 감지 정책** | 증분 시 `deleted_ids=[]` 하드코딩 | 증분=성능우선 생략, 비증분=전체스캔 감지로 명확화 |
| **사이드바 혼잡** | 임시보관함/지운편지함 불필요 | 제거 → 받은편지함+보낸편지함+폴더편지함으로 단순화 |
| **진행률 % 가시성** | QProgressBar 10px, text 없음 | 22px, text-align:center, bold |
| **OK 버튼 크기** | 112x34, border 1px | 150x38, border 2px |

### 9.2 신규 UI

| 항목 | 위치 | 설명 |
|---|---|---|
| `└ 폴더편지함` | 사이드바 > 받은편지함 아래 | 하위 폴더 이메일 합산 표시 |
| `🕐 최근 동기화` | 사이드바 > 동기화 카드 아래 | 마지막 동기화 시각, 클릭 시 동기화 |

### 9.3 변경 파일

```text
data/database.py          +66  (폴더명 별칭 중앙화)
core/sync_manager.py      +181 / -73  (동기화 버그 수정, 삭제/이동 감지)
core/outlook_connector.py +5 / -3  (Mock 증분 대응)
main.py                   +21 / -10  (별칭 중앙화, 버튼 확대)
ui/sidebar.py             +98 / -19  (폴더편지함, 최근 동기화)
ui/main_window.py         +26 / -9  (total_count, last_sync_time)
ui/settings_dialog.py     +31 / -11  (진행률 바, 버튼)
```
...
23개 파일 구문 체크: ALL OK
시나리오: 폴더 이동 ✅, 삭제 감지 ✅, 스캔 성능 ✅
```

> 상세 보고서: [OPTIMIZATION_REPORT.md](./OPTIMIZATION_REPORT.md)
